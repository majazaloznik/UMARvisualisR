...
dittodb::with_mock_db({
  ...
  test_that("transformations work as expected", {
    ...
    expect_equal(transform_rolling(datapoints[[3]])[6, 2][[1]], 4.073873667, tolerance = 1e-8)
    expect_equal(transform_rolling(datapoints[[3]], 3, "r")[7, 2][[1]], 4.073873667, tolerance = 1e-8)
    expect_equal(transform_rolling(datapoints[[3]], 3, "l")[5, 2][[1]], 4.073873667, tolerance = 1e-8)
    ...
    expect_equal(transform_growth(datapoints[[3]])[9,2][[1]], -94.4, tolerance = 1e-2)
    expect_equal(transform_growth(datapoints[[3]], "QOQ")[6,2][[1]], -97.1, tolerance = 1e-2)
    ...
    expect_warning(expect_equal(transform_index(datapoints[[1]], 1990)$df[1,2][[1]], 100))
    expect_warning(expect_equal(mean(transform_index(datapoints[[3]], 1990)$df[5:8,2][[1]]), 100))
    expect_equal(mean(transform_index(datapoints[[3]], "1996Q1")$df[5,2][[1]]), 100)
    expect_warning(expect_equal(mean(transform_index(datapoints[[3]], "1995Q1")$df[5,2][[1]]), 100))
